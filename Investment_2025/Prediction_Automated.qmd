...
print(data.frame(Date = index(last_5), IXIC.Close = round(Cl(last_5), 2)))
...
today_data <- full_data %>% arrange(date) %>% slice(n())  # last row
prediction_date <- today_data$date + 1                    # next market day

train_data <- full_data %>% filter(date < today_data$date)
...
cat("\n=== DAILY NASDAQ SIGNAL ===\n")
cat("Prediction is for market day:", as.character(prediction_date), "\n")
cat("Based on data available up to:", as.character(today_data$date), "\n")
...
print(tail(index(nasdaq_returns), 3))
```
