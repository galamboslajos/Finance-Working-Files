...
This document outlines the implementation of an Opening Range Breakout (ORB) strategy using 5-minute intraday data for the NASDAQ-100 ETF (QQQ). The strategy is designed to capture price movements based on the opening range of the first two 5-minute candles after market open and take position with a dynamic risk related sizing method.
...
`R` is derived from a volatility proxy. Defined as the distance between entry and stop; we set the stop a fixed multiple of the recent range, so `R` automatically grows when the market is more volatile and shrinks when it’s quieter. This implements classic volatility scaling: higher range ⇒ smaller size, lower range ⇒ larger size.
...
run_orb_backtest <- function(df,
                             ...
                             max_leverage         = 10,
                             spread_bps           = 0.3,       # full bid-ask spread (bps)
                             slippage_sd_bps      = 0.3,       # slippage σ (bps, adverse)
                             seed                 = NULL,
...
A single cumulative equity curve as a backtest result is naive. It points to a single historical realization, however, other realizations were possible too. This approach resamples blocks of consecutive trading days to create new datasets, preserving temporal dependencies*. We then re-run the backtest on each resampled dataset to evaluate performance variability.

The results show that the strategy is highly regime dependent; it relies almost fully on certain momentums in market. The fact that even 50 day blocks are not enough to stabilise the results indicates that the strategy is not robust in its current form. However, we might be able to achieve more stable results by building a more regime aware model, or alternatively except the regime dependece but scale positions down once on a loosing streak. 
...
# Get unique trading days for bootstrap
unique_days <- df %>%
  mutate(trading_date = as.Date(datetime)) %>%
  distinct(trading_date) %>%
  pull(trading_date)

n_days <- length(unique_days)
cat("Total trading days available:", n_days, "\n")

# Block Bootstrap Implementation
set.seed(123)
n_simulations <- 100
block_size <- 100  # 100-day blocks

# Create block bootstrap datasets
simulated_datasets_blocks <- list()

for (i in 1:n_simulations) {
  # Calculate how many blocks we need
  n_blocks_needed <- ceiling(n_days / block_size)
  
  sampled_blocks <- map(1:n_blocks_needed, function(x) {
    max_start <- max(1, n_days - block_size + 1)
    start_day <- sample(1:max_start, 1)
    end_day <- min(start_day + block_size - 1, n_days)
    unique_days[start_day:end_day]
  }) %>% 
  unlist()
  
  sampled_days <- sampled_blocks[1:min(length(sampled_blocks), n_days)]
  
  sim_df <- map_dfr(sampled_days, function(day) {
    df %>%
      mutate(trading_date = as.Date(datetime)) %>%
      filter(trading_date == day)
  }) %>%
  select(-trading_date) %>%
  arrange(datetime)
  
  simulated_datasets_blocks[[i]] <- sim_df
}

# Run backtests on all block bootstrap simulations
simulation_results_blocks <- map(simulated_datasets_blocks, ~run_orb_backtest(.x))

# Create equity curves for block bootstrap
equity_curves_blocks <- map_dfr(simulation_results_blocks, function(log) {
  if(nrow(log) == 0) return(tibble())
  
  log %>%
    mutate(trade_number = row_number()) %>%
    select(trade_number, CapitalAfter)
}, .id = "sim_id") %>%
  mutate(sim_id = as.numeric(sim_id))

# Extract summary statistics
simulation_summary_blocks <- map_dfr(simulation_results_blocks, function(log) {
  if(nrow(log) == 0) {
    return(tibble(
      total_trades = 0,
      total_pnl = 0,
  ...
  tibble(
    ...
    total_pnl = sum(log$NetPnL, na.rm = TRUE),
    final_capital = last(log$CapitalAfter),
    win_rate = mean(log$NetPnL > 0, na.rm = TRUE)
  )
}, .id = "sim_id")

# Plot block bootstrap results
p1_blocks <- equity_curves_blocks %>%
  ggplot(aes(x = trade_number, y = CapitalAfter, group = sim_id)) +
  geom_line(alpha = 0.3, color = "darkgreen") +
  geom_hline(yintercept = 100000, color = "red", linetype = "dashed") +
  labs(
    title = "ORB Strategy: 100 Block Bootstrap Realizations (100-day blocks)",
    x = "Trade Number",
    y = "Capital ($)",
    subtitle = "Starting capital: $100,000"
  ) +
  theme_minimal()

p2_blocks <- simulation_summary_blocks %>%
  ggplot(aes(x = final_capital)) +
  geom_histogram(bins = 20, fill = "darkgreen", alpha = 0.7) +
  geom_vline(xintercept = 100000, color = "red", linetype = "dashed") +
  geom_vline(xintercept = mean(simulation_summary_blocks$final_capital), 
             color = "blue", linetype = "dashed") +
  labs(
    title = "Block Bootstrap: Final Capital Distribution",
    x = "Final Capital ($)",
    y = "Count"
  ) +
  theme_minimal()
...
# Print statistics
cat("Block Bootstrap Results (100-day blocks):\n")
cat("Mean final capital:", round(mean(simulation_summary_blocks$final_capital)), "\n")
cat("Median final capital:", round(median(simulation_summary_blocks$final_capital)), "\n")
cat("Profitable simulations:", sum(simulation_summary_blocks$final_capital > 100000), 
    "out of", nrow(simulation_summary_blocks), "\n")
cat("Success rate:", round(sum(simulation_summary_blocks$final_capital > 100000) / 
                          nrow(simulation_summary_blocks) * 100, 1), "%\n")
```
...
#| message: false
#| echo: false
# Test the original dataset first
original_log <- run_orb_backtest(df)

# Check if original strategy is profitable
cat("Original Strategy Performance:\n")
cat("Total trades:", nrow(original_log), "\n")
cat("Final capital:", tail(original_log$CapitalAfter, 1), "\n")
cat("Total PnL:", sum(original_log$NetPnL), "\n")
cat("Win rate:", mean(original_log$NetPnL > 0) * 100, "%\n")

# Plot original equity curve
original_log %>%
  mutate(trade_number = row_number()) %>%
  ggplot(aes(x = trade_number, y = CapitalAfter/100000)) +
  geom_line() +
  labs(title = "Original ORB Strategy Performance", 
       x = "Trade Number", y = "Capital")
```
...
#| comment: NA
#  ──────────────────────────────────────────────────────────────────────────
#  CHANGE "eval: false" in order to let the code run and be rendered if needed!
#  Optimise `risk_per_trade_pct`  and  `holding_minutes_opt`
#  objective → maximise `total_return` as computed in summarize_orb_performance
#  ──────────────────────────────────────────────────────────────────────────
#  Prerequisites:
#    ▸ run_orb_backtest()               – already defined earlier
#    ▸ summarize_orb_performance()      – already defined earlier
#    ▸ df                               – minute-bar data frame in memory
#  ──────────────────────────────────────────────────────────────────────────

optimise_orb_params <- function(df,
                                risk_grid   = seq(0.0025, 0.0300, by = 0.0025),   # 0.25 % … 3 %
                                hold_grid   = seq(5,        390, by = 5),       
                                init_capital = 100000,
                                ...) {                                           # pass-through args
  
  # ── helper: run one combo & return total return ──────────────────────────
  eval_combo <- function(risk_pct, hold_min) {
    log <- run_orb_backtest(df,
                            initial_capital     = init_capital,
                            risk_per_trade_pct  = risk_pct,
                            holding_minutes_opt = hold_min,
                            ...)
    
    if (nrow(log) == 0) return(NA_real_)                        
    (tail(log$CapitalAfter, 1) - log$CapitalBefore[1]) /
      log$CapitalBefore[1]                                      
  }
  
  # ── grid search ──────────────────────────────────────────────────────────
  combos <- expand.grid(risk = risk_grid,
                        hold = hold_grid,
                        KEEP.OUT.ATTRS = FALSE,
                        stringsAsFactors = FALSE)
  
  pb <- txtProgressBar(min = 0, max = nrow(combos), style = 3)
  combos$total_return <- vapply(seq_len(nrow(combos)), function(i) {
    setTxtProgressBar(pb, i)
    eval_combo(combos$risk[i], combos$hold[i])
  }, numeric(1))
  close(pb)
  
  # ── best combo + full grid out ───────────────────────────────────────────
  best_row <- combos[which.max(combos$total_return), ]
  
  list(best_params = list(risk_per_trade_pct  = best_row$risk,
                          holding_minutes_opt = best_row$hold,
                          total_return        = best_row$total_return),
       full_grid   = combos)
}

# ── run the optimiser ───────────────────────────────────────────────────────
opt_results <- optimise_orb_params(df)

print(opt_results$best_params)

best_log <- run_orb_backtest(df,
                             risk_per_trade_pct  = opt_results$best_params$risk_per_trade_pct,
                             holding_minutes_opt = opt_results$best_params$holding_minutes_opt)

summarize_orb_performance(best_log, df)
...
Let's explore the hypothetical profit-taking moments in the trades, assuming we could exit at any 5-minute interval after entry. This will help us understand how holding time affects the R-multiple and whether it is rational to hold until the end of the day.
...
# ── 1.  KEEP *ALL* TRADES ──────────────────────────────────────────────
valid_trades_all <- valid_trades   # ← no ExitReason filter

# ── 2.  Build 5-min grid up to the real exit (TP or SL) ────────────────
expanded_trades <- valid_trades_all %>%
  rowwise() %>%
  mutate(
    HypoExitTimes = list(seq.POSIXt(EntryTime, ExitTime, by = "5 mins"))
  ) %>%
  unnest(HypoExitTimes) %>%
  filter(HypoExitTimes > EntryTime) %>%  # skip 0-minute row
  ungroup()
...
# ── 4.  Compute hypothetical P&L and R multiple ────────────────────────
expanded_trades <- expanded_trades %>%
  mutate(
...
# ── 5.  Trim extremes & plot (unchanged) ───────────────────────────────
...
ggplot(filtered_trades, aes(x = MinutesHeld, y = HypoRMultiple)) +
  geom_point(aes(color = "Individual trades"), alpha = 0.1) +
  stat_summary_bin(
    aes(color = "5 min median R"), 
    fun = median, bins = 60, geom = "line", linewidth = 1
  ) +
  # LOESS with linetype set outside of aes()
  geom_smooth(
    aes(color = "LOESS fit"), 
    method = "loess", se = FALSE, linetype = "dashed", linewidth = 0.8
  ) +
  scale_color_manual(
    name   = "Series",
    values = c(
      "Individual trades" = "grey70",
      "5 min median R"    = "purple",
      "LOESS fit"         = "red"
    )
  ) +
  labs(
    title    = "Hypothetical R Multiple vs. Holding Time",
    subtitle = "5 min by 5 min Holding Periods: median R multiples if sold in successive 5 min buckets",
    x        = "Minutes Held",
...
There is an apparent trade off between the number of trades that make it until the EoD vs the R multiple achieved. The longer the trades are held, the more likely they are to be closed at a loss. This is being shown in the next section, where the trades are bucketed into 5-minute bands and the R-multiple is shown as a box-plot. The bottom panel shows the number of trades in each bucket.
...
# ── 1.  Bucket the data in 5-minute bands ─────────────────────────────
bucketed <- filtered_trades %>%
  mutate(MinsBucket = cut(
    MinutesHeld,
    breaks = seq(0, max(MinutesHeld, na.rm = TRUE) + 5, by = 5),   # 0–5, 5–10, …
    include.lowest = TRUE, right  = FALSE,
    labels = paste0(
      seq(0, max(MinutesHeld, na.rm = TRUE), by = 5),  "–",
      seq(5, max(MinutesHeld, na.rm = TRUE) + 5, by = 5), " m"
    )
  ))

# ── 2.  Count observations in each bucket ─────────────────────────────
n_by_bucket <- bucketed %>%
  count(MinsBucket, name = "n")

# ── 3.  Build the box-plot (top panel) ────────────────────────────────
p_box <- ggplot(bucketed,
                aes(x = MinsBucket, y = HypoRMultiple)) +
  geom_boxplot(outlier.alpha = 0.05,
               fill = "skyblue",
               color = "darkblue") +
  labs(
    title = "Hypothetical R-Multiple by Holding-Time (5-min buckets)",
    y     = "R Multiple",
    x     = NULL   # hide x-axis title here so it shows only once
  ) +
  theme_minimal() +
  theme(
    axis.text.x = element_blank(),      # hide tick labels on top plot
    axis.ticks.x = element_blank()
  )

# ── 4.  Build the bar chart (bottom panel) ────────────────────────────
p_bar <- ggplot(n_by_bucket,
                aes(x = MinsBucket, y = n)) +
  geom_col(fill = "grey60") +
  labs(
    title = NULL,
    y     = "Number of trades",
    x     = "Minutes Held (5-min bands)"
  ) +
  theme_minimal() +
  theme(
...
# ── 5.  Stack the two plots vertically ────────────────────────────────
p_box / p_bar +           # patchwork’s “/” stacks vertically
  plot_layout(heights = c(3, 1))   # give the box-plot more space

```
Analytically, we can check for optimal holding period based on the backtest results by calculating the first derivative of the LOESS fit. This will help us identify the point where the R-multiple starts to decrease over holding period, indicating that holding longer is not beneficial. However, even visually, we can see that the R-multiple is strictly increasing. 
...
ggplot(df_loess_deriv, aes(x = MinutesHeld, y = dR_dt)) +
  ...
  theme_minimal()
```
We aslo look at the R-multiple per minute held, which is a measure of "efficiency" (in terms of time). The higher the R-multiple per minute, the more efficient the trade is. This is shown in the next plot, where we filter out trades that were held for 0 minutes (to avoid division by zero) and calculate the R-multiple per minute held.
...
We noticed during the real-life testing period, that the strategy's performance and win-rate was significantly (positively) affected by the fact that we eventually still deviate from the original strategy by not strictly adhering to the "End of the Day" holding rule. 