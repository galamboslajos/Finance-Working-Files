...
#### Change for "last_year" for the latest observations ####
df <- read_parquet("~/Desktop/qqq_5min.parquet")
head(df, 10)
tail(df, 10)
```
...
summarize_orb_performance <- function(trade_log, df, rf = 0.04) {
  ...
  # --- Performance metrics ---
  #### original total return calculation:  total_return <- (last(capital_curve) - first(trade_log$CapitalBefore)) / first(trade_log$CapitalBefore) ####
  total_return <- (last(capital_curve) / first(trade_log$CapitalBefore))
...
#| message: false
#| echo: false
#| fig-cap: "Rolling Sharpe Ratio and Net Monthly PnL"
#| fig-asp: 0.6
...
# ─────────────────────────────────────────────────────────────
# 2)  **Net** monthly PnL (sum of profits and losses)
# ─────────────────────────────────────────────────────────────
...
monthly_pnl <- log %>%
  ...
  group_by(Month) %>%
  summarise(
    NetPnL = sum(NetPnL),      # <-- netted‑out PnL, not average
    Trades = n(),
    .groups = "drop"
  )
...
# ─────────────────────────────────────────────────────────────
# 3)  Display the two plots one below the other
# ─────────────────────────────────────────────────────────────
