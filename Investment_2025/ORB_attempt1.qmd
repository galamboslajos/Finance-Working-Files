...
# Create 5-min intervals from entry to exit
...
# Only keep time & close columns
price_df <- df %>%
  select(datetime, close) %>%
  rename(HypoExitTimes = datetime, ExitPriceClose = close)

# Merge to get price at each hypothetical exit
...
ggplot(expanded_trades, aes(x = MinutesHeld, y = HypoRMultiple)) +
  stat_summary_bin(
    fun = median, bins = 60, geom = "line", color = "blue"
  ) +
  labs(
    title = "Hypothetical R Multiple by Holding Time (5-min resolution)",
    x = "Minutes Held Since Entry",
    y = "Median R Multiple"
  ) +
  theme_minimal()

ggplot(expanded_trades, aes(x = factor(MinutesHeld), y = HypoRMultiple)) +
  geom_boxplot(outlier.alpha = 0.1) +
  labs(
    title = "Distribution of Hypothetical R Multiples",
    x = "Minutes Held",
    y = "R Multiple"
  ) +
  theme_minimal() +
