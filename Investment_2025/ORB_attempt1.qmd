...
optimise_orb_params <- function(df,
                                ...
                                hold_grid   = seq(30,        390, by = 5),       # 30-min steps
                                init_capital = 100000,
                                ...) {                                           # pass-through args
  ...
  eval_combo <- function(risk_pct, hold_min) {
    ...
    if (nrow(log) == 0) return(NA_real_)                        # skipped day(s)
    (tail(log$CapitalAfter, 1) - log$CapitalBefore[1]) /
      log$CapitalBefore[1]                                      # total_return
  }
...
## Exploring Hypothetical Profit Taking Moments, Optimal Holding Period (ongoing)
```{r}
#| warning: false
#| message: false
#| echo: false
