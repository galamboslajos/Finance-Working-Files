...
``` r
# Filter data for dates after March 29, 2021
df <- df %>%
  filter(datetime > as.POSIXct("2021-03-24", tz = "UTC"))
```

    # A tibble: 17 × 2
       Metric                    Value       
       <chr>                     <chr>       
     1 Total Return (%)          445.0879    
     2 Annualized Return (%)     32.4913     
     3 Annualized Volatility (%) 46.5281     
     4 Annualized Sharpe Ratio   0.9806      
     5 Max Drawdown (%)          -30.6127    
     6 CAPM Alpha (%)            47.1425     
     7 Beta                      0.1672      
     8 Total Trades              1039.0000   
     9 Winning Trades            250.0000    
    10 Losing Trades             789.0000    
    11 Breakeven Trades          0.0000      
    12 Win Rate (%)              24.0616     
    13 Avg Net PnL               428.3810    
    14 Profit Factor             1.1935      
    15 Total Commission Paid     36734.0000  
    16 Avg Position Notional     1402550.3821
    17 Avg Leverage Used         4.9079      
...
## Strategy Robustness via Block Bootstrap Resampling

A single cumulative equity curve as a backtest result is naive. It
points to a single historical realization, however, other realizations
were possible too. This approach resamples blocks of consecutive trading
days to create new datasets, preserving temporal dependencies\*. We then
re-run the backtest on each resampled dataset to evaluate performance
variability.

The results show that the strategy is highly regime dependent; it relies
almost fully on certain momentums in market. The fact that even 50 day
blocks are not enough to stabilise the results indicates that the
strategy is not robust in its current form. However, we might be able to
achieve more stable results by building a more regime aware model, or
alternatively except the regime dependece but scale positions down once
on a loosing streak.

\*if we use large enough windows.

    Total trading days available: 1046 

![](ORB_attempt1.markdown_strict_files/figure-markdown_strict/unnamed-chunk-9-1.png)

![](ORB_attempt1.markdown_strict_files/figure-markdown_strict/unnamed-chunk-9-2.png)

    Block Bootstrap Results (100-day blocks):

    Mean final capital: 28287 

    Median final capital: 24212 

    Profitable simulations: 0 out of 100 
...
### We might observe different subsamples of time-periods separatley in cummulative terms, but it will not be as informative about robustness as the Block-Bootstrap approach above.
...
Rate](ORB_attempt1.markdown_strict_files/figure-markdown_strict/unnamed-chunk-13-1.png)
...
candle’s closing price level (conversely in a long-case).
    ...
    Directional success count: 355 out of 701 trading days

    Directional accuracy rate: 50.64 %
...
Legend](ORB_attempt1.markdown_strict_files/figure-markdown_strict/unnamed-chunk-16-1.png)
...
![](ORB_attempt1.markdown_strict_files/figure-markdown_strict/unnamed-chunk-19-1.png)
...
![](ORB_attempt1.markdown_strict_files/figure-markdown_strict/unnamed-chunk-20-1.png)
...
![](ORB_attempt1.markdown_strict_files/figure-markdown_strict/unnamed-chunk-21-1.png)
