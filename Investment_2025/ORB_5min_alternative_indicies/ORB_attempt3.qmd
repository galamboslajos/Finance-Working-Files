...
# Create a 30-minute floor timestamp
df <- df %>%
  mutate(datetime_30min = floor_date(datetime, unit = "30 minutes"))

df <- df %>%
  mutate(datetime_30min = floor_date(datetime, unit = "30 minutes")) %>%
  group_by(Ticker, datetime_30min) %>%
  ...
  rename(datetime = datetime_30min) %>%
  arrange(datetime)
```
